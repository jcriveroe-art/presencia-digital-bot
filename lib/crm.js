...
async function logMensaje(telefono, direccion, mensaje, raw, wamid) {
  const payload = { telefono, direccion, mensaje, raw: raw || null };
  try {
    if (wamid) {
      const { data, error } = await supabase
        .from("mensajes")
        .upsert({ ...payload, wamid }, { onConflict: "wamid", ignoreDuplicates: true })
        .select("id");
      if (error) {
      ...
      return { guardado: !duplicado, duplicado };
    }
    ...
    return wamid ? { guardado: false, duplicado: false } : false;
  }
}
